...
func act(grid map[image.Point]int, inst string) {
...
func printGrid(grid map[image.Point]int) {
	n := gridSize(grid)
	for y := range n {
		for x := range n {
			fmt.Print(grid[image.Pt(x, y)], " ")
		}
		fmt.Println()
	}
	fmt.Println("--------")
...
func main() {
	...
	instructions := strings.Split(string(parts[1]), "\n")
	var taken string
	for {
		for _, action := range bytes.Fields(parts[2]) {
			switch action[0] {
			case 'T':
				taken = instructions[0]
			...
			case 'C':
				instructions = append(instructions, taken)
			}
		}
	}
done:
